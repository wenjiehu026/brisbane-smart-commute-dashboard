...
DEFAULT_TRANSLINK_BASE_URL = "https://gtfsrt.api.translink.com.au/api/realtime/SEQ"
...
class Settings:
    ...
    vehicle_positions_url: str = os.getenv(
        ...
        f"{DEFAULT_TRANSLINK_BASE_URL}/VehiclePositions",
    )
    trip_updates_url: str = os.getenv(
        "TRANSLINK_TRIP_UPDATES_URL",
        f"{DEFAULT_TRANSLINK_BASE_URL}/TripUpdates",
    )
    service_alerts_url: str = os.getenv(
        "TRANSLINK_SERVICE_ALERTS_URL",
        f"{DEFAULT_TRANSLINK_BASE_URL}/Alerts",
    )
    ingestion_interval_seconds: int = int(os.getenv("INGESTION_INTERVAL_SECONDS", "60"))
    enable_live_ingestion: bool = os.getenv("ENABLE_LIVE_INGESTION", "true").lower() == "true"
